<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-bold text-2xl text-[#111827]">Tambah Data Siswa</h2>
            <p class="text-sm text-slate-500 mt-1">Tambahkan data siswa baru.</p>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-[2rem] shadow border border-[#F5E7B2] overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#111827] to-[#7A5C1E] px-8 py-6 text-white">
                <h3 class="text-xl font-bold">Form Tambah Siswa</h3>
                <p class="text-sm text-[#F5E7B2]">Lengkapi data siswa.</p>
            </div>

            <form action="{{ route('admin.siswa.store') }}" method="POST" class="p-8">
                @csrf
